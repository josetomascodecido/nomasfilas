...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post locals_url, params: { local: { name: @local.name, razonsocial: @local.razonsocial, rut: @local.rut } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { name: @local.name, razonsocial: @local.razonsocial, rut: @local.rut } }
